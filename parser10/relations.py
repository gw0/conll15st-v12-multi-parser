...
if __name__ == '__main__':
    ...
    argp.add_argument('xxx', nargs='*',
        help="XXX")
    args = argp.parse_args()

    # load relations by document id
    relations = conll15st_relations.load_relations(args.dataset_dir)
    relations = conll15st_relations.conv_tokenlists(relations)

    # load words by document id
    words = conll15st_words.load_words(args.dataset_dir)
    words = conll15st_relations.conv_linkers_to_tags(words, relations)

    doc_ids = args.xxx
    print doc_ids
    relations = dict([ (i, relations[i]) for i in doc_ids ])
    words = dict([ (i, words[i]) for i in doc_ids ])
    ...
    for doc_id in relations:
        ...
        #print(doc_id)
        #print("transform back to relations...")
        relations2 = to_relations({ doc_id: pairs[doc_id] }, { doc_id: words[doc_id] })
        #print("verify results...")
        ...
        for r1 in relations[doc_id]:
            for r2 in relations2[doc_id]:
        ...
        print(doc_id)
        #print("match: {} (extr: {}) (orig: {})".format(cnt_match, sum([ len(r)  for r in relations2.itervalues() ]), sum([ len(r)  for r in relations.itervalues() ])))
        print("match: {} (extr: {}) (orig: {})".format(cnt_match, len(relations2[doc_id]), len(relations[doc_id])))
